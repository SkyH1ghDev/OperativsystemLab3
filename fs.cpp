...
int FS::create(std::string const &filepath) {
    ...
    dir_entry dirEntry = MakeDirEntry(filename, size, indexVector.at(0), TYPE_FILE, READ | WRITE);
...
int FS::cat(std::string const &filepath) {
    ...
    if ((filePtr->access_rights & READ) == 0)
    {
        std::cout << "You don't have the privilege to read this file" << std::endl;
        return -1;
    }
...
int FS::ls() {
    ...
    for (dir_entry &file: this->directoryTreeWorkingDirectoryPtr->value) {
        ...
        std::cout.width(20);
        std::cout << (file.type == TYPE_DIR ? "Dir" : "File");
        std::cout.width(20);
        std::cout << rightsMap[file.access_rights];
        std::cout.width(20);
        std::cout << (file.type == TYPE_DIR ? "-" : std::to_string(file.size));
        std::cout.width(20);
        std::cout << file.first_blk << std::endl;
    }
...
int FS::cp(std::string sourcepath, std::string destpath)
{
    ...
    FilepathType filepathType = destinationPath[0] == '/' ? Absolute : Relative;
    TreeNode<std::vector<dir_entry> > *directoryNodePtr;
...
int FS::append(std::string filepath1, std::string filepath2) {
    ...
    if ((sourceFilePtr->access_rights & READ) == 0)
    {
        std::cout << "You don't have the privilege to read the source file" << std::endl;
        return -1;
    }
    ...
    if ((destFilePtr->access_rights & WRITE) == 0) {
        std::cout << "You don't have the privilege to write to the destination file" << std::endl;
        return -1;
    }
